...
export function RunPanel({ initial }: RunPanelProps) {
  const { state, phase, lastError } = useThreadPoll(initial.thread_id);
    ...
    <Card data-testid="run-panel">
      <CardHeader className="flex flex-row items-start justify-between gap-4">
        <div>
          ...
          <CardDescription>
            Thread <code className="font-mono text-xs">{current.thread_id}</code>
          </CardDescription>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Checkpoint and result panels land in US-19 onward. In the meantime
          the legacy{" "}
