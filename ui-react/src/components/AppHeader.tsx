...
// lucide-react dropped its brand icons (no `Github` export in v1.x), so the
// recognisable mark is inlined as an SVG rather than swapped for a generic glyph.
function GithubMark() {
  return (
...
/**
 * Sticky top bar: wordmark + theme toggle + repo link. The orange block
 * mark reuses the --primary brand token so the identity matches the
 * accent used on primary actions throughout the app.
 */
    ...
    <header className="sticky top-0 z-30 border-b border-border/60 bg-background/80 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="mx-auto flex h-14 max-w-[1100px] items-center justify-between px-4">
        <a href="/" className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-semibold tracking-tight">RTIA</span>
        </a>
